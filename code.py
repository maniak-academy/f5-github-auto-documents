...
# Lists to keep track of all virtual servers and pool members
all_virtual_server_stats = []
all_pool_member_stats = []
problematic_vs = []
    ...
    if auth_token:
        ...
        for vs in virtual_servers:
            ...
            try:
                ...
                all_virtual_server_stats.append(vs_stat)  # Collecting all virtual servers
            except Exception as e:
            ...
            if pool_name and pool_name != 'No Pool':
                ...
                try:
                    ...
                    for member in pool_members_response.get('items', []):
                        ...
                        all_pool_member_stats.append(member_stat)  # Collecting all pool members
                except Exception as e:
...
md_output_file = "f5_virtual_servers_and_pools.md"

# Start building the Markdown content
md_content = "# F5 Virtual Servers and Pool Members Overview\n\n"

# Create a table of all virtual servers with links to their detailed sections
md_content += "## Virtual Servers Overview\n\n"
md_content += "| Device | Hostname | Virtual Server Name | Destination | IP Protocol | Enabled | Status | Pool |\n"
md_content += "|--------|----------|---------------------|-------------|-------------|---------|--------|------|\n"

for vs_stat in all_virtual_server_stats:
    # Create an anchor link for each virtual server based on its name
    vs_name_anchor = vs_stat['VirtualServerName'].replace('/', '_').replace(' ', '_')
    md_content += f"| {vs_stat['Device']} | {vs_stat['Hostname']} | [{vs_stat['VirtualServerName']}](#{vs_name_anchor}) | {vs_stat['Destination']} | {vs_stat['IPProtocol']} | {vs_stat['Enabled']} | {vs_stat['Status']} | {vs_stat['Pool']} |\n"

md_content += "\n"

# Add detailed sections for each virtual server's pool members
md_content += "## Virtual Server Pool Member Details\n\n"

# Group pool members by virtual server
pool_members_by_vs = defaultdict(list)
for member_stat in all_pool_member_stats:
    vs_name = member_stat['VirtualServerName']
    pool_members_by_vs[vs_name].append(member_stat)

for vs_stat in all_virtual_server_stats:
    vs_name = vs_stat['VirtualServerName']
    vs_name_anchor = vs_name.replace('/', '_').replace(' ', '_')
    md_content += f"### {vs_stat['VirtualServerName']} ({vs_stat['Hostname']} - {vs_stat['Device']})\n"
    md_content += f"<a id='{vs_name_anchor}'></a>\n\n"
    pool_members = pool_members_by_vs.get(vs_name, [])
    if pool_members:
